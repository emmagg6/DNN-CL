...
# Adapted / extended to Fashion mnist by emmagg6
...
def mnistdataset(is_trn, n_per_class):
...
########################################################### 
# fmnistdataset -- creation of identical file form
###########################################################
...
def preprocess_fashion_mnist():
    ...
    data = scipy.sparse.csr_matrix(np.hstack((images, labels[:, np.newaxis])))
    scipy.sparse.save_npz('../datasets/img_captchas/fashion_mnist_784.npz', data)

# === DATASET LOADING === comment out if already loaded or not needed
# preprocess_fashion_mnist() # E


########################################################### 
# fmnistdataset -- load dataset
###########################################################

def fmnistdataset(is_trn, n_per_class):
    ...
    data = scipy.sparse.load_npz('../datasets/img_captchas/fashion_mnist_784.npz')  # load Fashion MNIST data
    img_sz = [28, 28]  # MUST be square for now
    image_data = data[:, :-1]
    label_data = np.array(data[:, -1].todense()).ravel()

    if is_trn:  # load training data
        data = image_data[:60000]
        labels = label_data[:60000]
    else:  # load test data
        data = image_data[60000:]
    ...
    for label in uniq_labels:
        ...
        class_data.append(curr_data[idx[:n_per_class]])
        class_labels.append(curr_labels[idx[:n_per_class]])
